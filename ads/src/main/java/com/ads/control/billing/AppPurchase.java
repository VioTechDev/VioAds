...
public class AppPurchase {
            ...
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                ...
                if (listSubscriptionId.size() > 0) {
                    QueryProductDetailsParams paramsSUBS = QueryProductDetailsParams.newBuilder()
                            ...
                            .build();
                    for (QueryProductDetailsParams.Product item : listSubscriptionId) {
    ...
    public void initBilling(final Application application, List<PurchaseItem> purchaseItemList) {
        if (AppUtil.VARIANT_DEV) {
            // auto add purchase test when dev
...
/*    private String logResultBilling(Purchase.PurchasesResult result) {
    ...
    public String purchase(Activity activity, String productId) {
        ...
        if (productDetails != null) {
            ...
            Log.d(TAG, "purchase: something error with id: " + productId);
        }
    ...
    public String subscribe(Activity activity, String SubsId) {
        ...
        for (PurchaseItem item : purchaseItems) {
            if (item.getItemId().equals(SubsId)) {
                trailId = item.getTrialId();
                break;
            }
        }
        String offerToken = "";
        for (ProductDetails.SubscriptionOfferDetails item : subsDetail) {
            String offerId = item.getOfferId();
            if (offerId != null && offerId.equals(trailId)) {
                offerToken = item.getOfferToken();
                break;
            }
        }
        if (offerToken.isEmpty()) {
            offerToken = subsDetail.get(subsDetail.size() - 1).getOfferToken();
        }
    ...
    private void handlePurchase(Purchase purchase) {
        ...
        VioLogEventManager.onTrackRevenuePurchase((float) price, currency, idPurchaseCurrent, typeIap);
...
//    }
    ...
    private void syncPurchaseItemsToListProduct(List<PurchaseItem> purchaseItems) {
        ArrayList<QueryProductDetailsParams.Product> listInAppProduct = new ArrayList<>();
        ArrayList<QueryProductDetailsParams.Product> listSubsProduct = new ArrayList<>();
        for (PurchaseItem item : purchaseItems) {
            QueryProductDetailsParams.Product product;
            if (item.getType() == TYPE_IAP.PURCHASE) {
